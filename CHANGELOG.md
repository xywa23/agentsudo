...
### Fixed
- Cloud telemetry HTTPS requests now use `certifi` when available to avoid macOS SSL certificate verification failures

## [0.3.1] - 2026-01-28

### Fixed
- `@sudo` decorator now properly wraps LangChain `@tool` decorated functions
- Fixed `NameError: name 'Annotated' is not defined` when using `@sudo` with LangChain tools
- Decorator now detects `StructuredTool` objects and wraps the underlying function while preserving tool metadata

## [0.1.2] - 2025-11-24

### Changed
- Updated README with comprehensive documentation and examples
- Removed "coming soon" placeholders for completed documentation

### Fixed
- PyPI package description now shows updated README
